...
Field operations teams have traditionally relied on **manual, paper-based checklists** to track worker tasks — like the washroom cleaning checklist used by Jubilant Cleaning Services Limited:
...
| **Task Dispatch Pro** | Frontend for task dispatch & management | `6057` |
| **Worker Insight Hub** | Frontend for analytics & worker insights | `5679` |
...
- 👤 **Admin Dashboard** — Auto-seeded admin account on first run
- 📬 **SMS Notifications** — Real-time alerts via Africa's Talking API
- ⚙️ **Background Jobs** — Redis-backed Celery with beat scheduler running every minute
- 📊 **Analytics Dashboard** — Worker performance insights and reporting
- 🗂️ **Structured Logging** — Per-service log files for easy debugging
...
- [🐳 Docker Deployment](#-docker-deployment) — recommended, fully containerized
- [💻 Local Setup](#-local-setup) — run services directly on your machine
...
> Replace `your_db_password`, `your_api_key_here`, and `your_secret_key_here` with your actual credentials.
...
1. Start a MariaDB database and Redis instance
2. Run Alembic migrations
3. Seed the admin user
4. Launch the FastAPI backend
5. Start the Celery worker and beat scheduler
6. Launch both frontend dashboards
...
| **Backend API** | http://localhost:8000 |
| **API Docs (Swagger)** | http://localhost:8000/docs |
| **Task Dispatch Pro** | http://localhost:6057 |
| **Worker Insight Hub** | http://localhost:5679 |
...
Available services: `api`, `celery`, `celery-beat`, `migrate`, `seed-admin`, `db`, `redis`, `task-dispatch-pro`, `worker-insight-hub`

### Local
All services write logs to dedicated files:
...
- The `backend/.env` file **must exist** before running either `run.sh` or `docker compose up`.
- For Docker deployment, set `DB_HOST=db` in your `.env` to point to the database container.
- For local deployment, the SQLite database (`database.db`) lives inside `backend/` and is created automatically.
- Frontends are launched **last** intentionally, ensuring the backend and Celery are fully initialized first.
- Update your Africa's Talking credentials in `.env` to enable live SMS notifications. Use `sandbox` mode for testing.
- When resetting the Docker database, always use `docker compose down -v` to remove the volume — otherwise MariaDB will skip reinitialization.
